...
import "@chainlink/contracts/src/v0.8/VRFConsumerBaseV2.sol";
  ...
  //to simplify funding and reduce the overall cost. Otherwise, the direct funding method. 
...
contract YourContract is VRFConsumerBaseV2,ERC721Enumerable, Ownable {
  ...
  //instantiating mappinga to store the random numbers that will be used to determine tbe characteristics of the nft (color, shape and speed)
...
//defining a requestRandomWords function that takes in a uint guess.
  function requestRandomWords(uint _guess) public payable {
    ...
    //only allow request if there if the available LINK is greater than 4 LINK. Since the fulfilment action occurs approximately 60 seconds after the request.
    if (getsubbalance() < 4) {
    ...
    //calling the requestRandomWords function from the ChainlinkClient class.
  ...
  function fulfillRandomWords(
   ...
   //emit the Guesses event for full payout
    ...
    //if the user has not guressed the correct number or is within 10 points of the correct number, the function checks is the random number delivered by the oracle is a prime number. If it is, the user is eligible to mint an NFT.
